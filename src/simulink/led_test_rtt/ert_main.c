...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(led_test_M) == (NULL)) && !rtmGetStopRequested
         (led_test_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(led_test_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(led_test_M, TRUE);
      }

      if (rtmGetStopRequested(led_test_M) == TRUE) {
    ...
    {
      boolean_T rtmStopReq = FALSE;
      rtExtModeOneStep(led_test_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(led_test_M, TRUE);
      }
    }
    ...
    {                                  /* Sample time: [0.2s, 0.0s] */
      rtExtModeUpload(0, led_test_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(led_test_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = FALSE;
    rtExtModeWaitForStartPkt(led_test_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(led_test_M, TRUE);
    }
  }
  ...
  if (rtmGetErrorStatus(led_test_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(led_test_M));
  }
