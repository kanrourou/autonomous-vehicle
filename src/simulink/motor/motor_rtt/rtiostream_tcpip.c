...
#define UDP_SOCKET_RECEIVE_SIZE_REQUEST (512 * 1024) 
...
static SOCKET serverOpenSocket(int port, char * serverInfoFile, CommsProtocol protocol);

#if (!defined(VXWORKS))
static SOCKET clientOpenSocket(char * hostName, unsigned int portNum, CommsProtocol protocol);
#endif
...
static int initConnectionData(int connectionID, 
   ...
   if (protocol == UDP_PROTOCOL) {      
      ...
      connection->udpData = malloc(sizeof(UDPData));
   ...
   if (isServer) {
      ...
      connection->serverData = malloc(sizeof(ServerData));
   ...
   if (isVerbose) {
      ...
      }
      {
         ...
         printf("Connection id %d, socket receive buffer size: %d\n", connectionID, optionValue);
      }
...
static UDPPacketBuffer * createUDPPacketBuffer(int maxPacketSize) {
   ...
   UDPPacketBuffer * udpPacketBuffer = malloc(sizeof(UDPPacketBuffer));      
   if (udpPacketBuffer == NULL) {
      printf("createUDPPacketBuffer:UDPPacketBuffer malloc failed.\n");
      return udpPacketBuffer; 
   }
   /* allocate the buffer */
   udpPacketBuffer->buffer = calloc(maxPacketSize, sizeof(char));
...
static SOCKET serverOpenSocket(int port, char * serverInfoFile, CommsProtocol protocol)
{
    ...
    if (lFd == INVALID_SOCKET) {
       ...
       if (protocol == TCP_PROTOCOL) {          
          ...
          /* increase the UDP socket receive size to decrease the 
           * possibility of buffer overflow */
          option = UDP_SOCKET_RECEIVE_SIZE_REQUEST;
          sockStatus = 
             setsockopt(lFd, SOL_SOCKET, SO_RCVBUF,(char*)&option, sizeof(option));
          if (sockStatus == SOCK_ERR) { 
...
static SOCKET clientOpenSocket(char * hostName, unsigned int portNum, CommsProtocol protocol) {
    ...
    if ((errStatus!=RTIOSTREAM_ERROR) && 
       ...
       /* increase the UDP socket receive size to decrease the 
        * possibility of buffer overflow */
       int option = UDP_SOCKET_RECEIVE_SIZE_REQUEST;
       int sockStatus = 
          setsockopt(cSock, SOL_SOCKET, SO_RCVBUF,(char*)&option, sizeof(option));
       if (sockStatus == SOCK_ERR) { 
          printf("setsocketopt() SO_RCVBUF call failed.\n");
...
int rtIOStreamOpen(int argc, void * argv[])
{
    ...
    if (result != RTIOSTREAM_ERROR) {
        if (isClient == 1) {
           ...
           sock = clientOpenSocket(xHostName, xPortNum, protocol);
           if (sock == INVALID_SOCKET) {
              result = RTIOSTREAM_ERROR;
           }
#endif
        } else {           
           sock = serverOpenSocket(xPortNum, serverInfoFile, protocol);            
           if (sock == INVALID_SOCKET) {
              result = RTIOSTREAM_ERROR;
           }
        }   
